    ...
    class S {
      ...
      _getHeaders() {
        ...
        return {
          ...
          "X-User-Name": ((r = this.user) == null ? void 0 : r.name) || "",
       ...
       * Handle API response
       */
      async _handleResponse(t) {
        if (!t.ok) {
          const e = await t.json().catch(() => ({})), r = new Error(e.error || `HTTP error ${t.status}`);
          throw r.status = t.status, r.data = e, r;
        }
        return t.json();
      }
      /**
       ...
       */
      async getFeatures() {
        const t = await fetch(`${this.baseUrl}/features`, {
          method: "GET",
      ...
      async createFeature(t) {
        const e = await fetch(`${this.baseUrl}/features`, {
          method: "POST",
          headers: this._getHeaders(),
          body: JSON.stringify({
            title: t.title,
    ...
    class v {
      ...
      renderCard(t) {
        ...
        <div class="wfl-card" data-id="${t.id}">
          <div class="wfl-vote">
            <button class="wfl-vote-btn wfl-vote-up wfl-tooltip ${r ? "wfl-voted" : ""}"
                    data-id="${t.id}"
                    data-action="up"
      ...
      attachEventListeners() {
        ...
        const r = this.container.querySelector("#wfl-comment-modal");
        if (r) {
      ...
      async openCommentModal(t) {
        ...
        try {
          ...
          }, u = this.container.querySelector("#wfl-comment-submit"), h = this.container.querySelector("#wfl-comment-input"), l = u.cloneNode(!0);
          u.parentNode.replaceChild(l, u), l.addEventListener("click", m);
      ...
      async handleVote(t) {
        ...
        if (!o) return;
        const i = this.container.querySelector(`.wfl-card[data-id="${e}"]`), s = i.querySelector(".wfl-vote-count"), a = i.querySelector(".wfl-vote-up"), m = i.querySelector(".wfl-vote-down");
        a.disabled = !0, m.disabled = !0;
        const u = o.votes, h = o.userVote;
        let l = "none", f = 0;
        r === "up" ? o.userVote === "up" ? (f = -1, l = "none") : o.userVote === "down" ? (f = 2, l = "up") : (f = 1, l = "up") : o.userVote === "down" ? (f = 1, l = "none") : o.userVote === "up" ? (f = -2, l = "down") : (f = -1, l = "down"), o.votes += f, o.userVote = l === "none" ? null : l, a.classList.toggle("wfl-voted", o.userVote === "up"), m.classList.toggle("wfl-voted", o.userVote === "down"), s.textContent = o.votes, s.classList.remove("wfl-vote-positive", "wfl-vote-negative"), o.votes > 0 ? s.classList.add("wfl-vote-positive") : o.votes < 0 && s.classList.add("wfl-vote-negative"), s.classList.add("wfl-animating"), setTimeout(() => s.classList.remove("wfl-animating"), 300), r === "up" && l === "up" && this.createConfetti(a);
        try {
          ...
          console.error("WPFeatureLoop: Failed to save vote", n), o.votes = u, o.userVote = h, a.classList.toggle("wfl-voted", o.userVote === "up"), m.classList.toggle("wfl-voted", o.userVote === "down"), s.textContent = o.votes, s.classList.remove("wfl-vote-positive", "wfl-vote-negative"), o.votes > 0 ? s.classList.add("wfl-vote-positive") : o.votes < 0 && s.classList.add("wfl-vote-negative"), this.showToast(this.t.errorText, "error");
        } finally {
          a.disabled = !1, m.disabled = !1;
        }
      }
      /**
       ...
       */
      createConfetti(t) {
        const e = ["#3b82f6", "#2563eb", "#1d4ed8", "#10b981", "#f59e0b"], r = t.getBoundingClientRect();
        for (let o = 0; o < 6; o++) {
      ...
      showToast(t, e = "default") {
        const r = this.container.querySelector("#wfl-toast");
        r && (r.textContent = t, r.className = "wfl-toast wfl-active", e === "success" ? r.classList.add("wfl-toast-success") : e === "error" && r.classList.add("wfl-toast-error"), setTimeout(() => {
          r.classList.remove("wfl-active");
        }, 3e3));
      }
      /**
       * Get all features
